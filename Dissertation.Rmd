...
ggplot(data, aes(x = Unique_Period))+
  ...
  geom_line(aes(y = GDP_SA, color = "GDP_SA"), size = 1) +
  labs(
    x = "Date ID",
    y = "Amount (? in millions)",
    title = "Revenue and Expenditure Over Time",
  ...
  scale_color_manual(values = c("GDP" = "blue", "GDP_SA" = "red")) +
  theme_minimal(base_size = 15) +
  theme(
    axis.text.x = element_text(angle = 45, hjust = 1),
    plot.title = element_text(hjust = 0.5, face = "bold"),
...
irf_result <- irf(svar_model, n.ahead = 10, ci = 0.9, boot = 500, cumulative = FALSE)  # Forecast horizons
# plot(irf_result)
# Visualize IRFs
