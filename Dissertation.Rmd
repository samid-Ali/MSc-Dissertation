...
When correctly specified, this representation can yield efficiency gains: including the error correction term may allow the model to capture important dynamics in the long run relationships between variables. We therefore proceed by testing for cointegration relationships in the data. 

Hubrich, Lutkepogl, and Saikkonen (2001) provide a review of cointegration tests. Conducting a monte carlo simulation, they find that the Likelihood Ratio type tests are among the best performing in practice, with other classes of tests having worse power or size. Hubrich, Lutkepohl, and Saikkonen (1998) highlight that improperly specified deterministic terms invalidate cointegration testing, with reduced power. Martin, Hurn, and Harris (2013) highlight that a constant deterministic component implies a linear trend in the VAR, thus we account for this in our tests for cointegration. 

Gonzalo and Lee (1998) analyse the robustness of test for cointegration, finding cases where the Johansen LR test for cointegration detects spurious cointegration. While they find that the Engle and Granger test is typically more robust, they propose that both tests are used. 
...
Given that we have performed cointegration tests, it would suggest that the appropriate model has been chosen, and thus we would expect the stability of the VAR model. This is confirmed by inspecting the eigenvalues of the VAR model, the greatest of which is `r round(max(roots(reduced_VAR)),2)` suggesting that we do not have issues of explosive roots. We therefore proceed by presenting the results of the SVAR analyis, focusing on Forecast Error Variance Decompositions (FEVDs) and Impulse Response Funcitions (IRFs)
...
colnames(rev_irf_df) <- c("Quarter","Expenditure", "LB", "UB")
...
The confidence intervals are obtained using a bootstrap approach. Kilian and Lutkepohl highlight the poor performance of asymptotic formulae (delta method) for impulse response functions. The T 
The IRF function in R computes Efron's (YYYY) percentile interval for impulse response functions.
*Explain Efron's PI*
It should be noted since it is constructed using a bootstrap distribution, this interval is not necessarily symmetric about the point estimate. Additionally, this method may be susceptible to bias in VAR estimates ... Augmentations have been proposed to correct for this bias. 

Here we focus on the response of GDP to the fiscal variables, the appendix includes comprehensive details of the IRFs for each of the variables.
...
- explain the bootstrap procedure/ why this may not be symmetric/ limitations
