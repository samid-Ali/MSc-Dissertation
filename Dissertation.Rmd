...
OptimalLag <-  VARselect(clean_data[,-c(1,2)], lag.max = 5, type = "both")
...
reduced_VAR <- VAR(clean_data[,-c(1,2)], p = 4, type = "both")
...
1) Blanchard and Perotti (2002) argue that the use of quarterly data allows government spending to be interpreted as predetermined with respect to the rest of the variables within the quarter. This is motivated by implementation lags for changes to government spedning and consequently this is ordered first.  
...
# Split data by pre- and post-2008 periods
clean_data1 <- model_data %>% na.omit() %>% filter(Year < 2008)
clean_data2 <- model_data %>% na.omit() %>% filter(Year >= 2008)

# Determine optimal lag selection for VAR models
OptimalLag1 <- VARselect(clean_data1[,-c(1,2)], lag.max = 5, type = "trend")$selection
OptimalLag2 <- VARselect(clean_data2[,-c(1,2)], lag.max = 5, type = "trend")$selection

# Estimate reduced-form VAR models
reduced_VAR1 <- VAR(clean_data1[,-c(1,2)], p = 4, type = "both")
reduced_VAR2 <- VAR(clean_data2[,-c(1,2)], p = 4, type = "both")

# Check stability of estimated VAR models
roots(reduced_VAR1)
roots(reduced_VAR2)

# Define recursive identification matrix
Amat <- matrix(c(1, 0, 0, 0, 0, 0,   
                 NA, 1, 0, 0, 0, 0,  
                 NA, NA, 1, 0, 0, 0,  
                 NA, NA, NA, 1, 0, 0,  
                 NA, NA, NA, NA, 1, 0,
...
# Estimate SVAR models with recursive restrictions
svar_model1 <- SVAR(reduced_VAR1, Amat = Amat, estmethod = "direct")
svar_model2 <- SVAR(reduced_VAR2, Amat = Amat, estmethod = "direct")

# Compute impulse response functions (IRFs)
irf_result1 <- irf(svar_model1, n.ahead = 10, ci = 0.9, boot = 500, cumulative = FALSE)
irf_result2 <- irf(svar_model2, n.ahead = 10, ci = 0.9, boot = 500, cumulative = FALSE)

# Compute forecast error variance decomposition (FEVD)
FEVD_result1 <- fevd(svar_model1, n.ahead = 10)
FEVD_result2 <- fevd(svar_model2, n.ahead = 10)
...
plot_irf <- function(irf_long) {
  ...
  for (shock in shock_names) {
    ...
    print(p)  # Display each figure separately
  }
}
...
irf_long1 <- process_irf(irf_result1)
