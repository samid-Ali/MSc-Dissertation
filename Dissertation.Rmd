...
model to include also a short term interest rate, the GDP deflator rate, and UK exports. These variables are included to account for the effects of monetary policy, and 
...
Following Fernandez (2006), the natural logarithm of these variables is used, with the exception of the short term interest rate, which enters the model in levels. Furthermore, the fiscal variables and GDP are used in real terms. Variables have been seasonally adjusted using X-13ARIMA-SEATS (with the exception of GDP which was sourced after seasonal adjustment). The intention of this is tease out the noise from the data so that the model can estimate the underlying trends. The model includes a trend component to account for ... Gechert (2017) highlight the importance of  
