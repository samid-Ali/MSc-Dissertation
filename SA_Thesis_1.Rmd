...
Ramey (2019) surveys the literature on fiscal multipliers, among the empirical approaches for estimating the effects of fiscal policy are SVARs and DSGE models. Ramey highlights the identification of exogenous variation as a challenge for both of these methodologies.
In this review Ramey highlights variation in estimates for the fiscal multipliers, with ranges for government spending varying from 0.3 to 2. Ramey highlights that tax multipliers are typically negative, howver with much greater variability, reporting a range from 0 for corporate income tax in an estimated DSGE model, to -5 in for overall taxes in an SVAR identified using sign restrictions. .
...
# Attempt to construct present value multipliers
...
ggplot(exp_irf_df, aes(x = Quarter)) +
  ...
  labs(
    title = "Cumulative Impulse Response Function",
    x = "Quarter",
...
Here we focus on the response of GDP to the fiscal variables, figure 1 summarises the results for a transitory shock to government expenditure. Given the model specification, the results are interpreted as elasticities in terms of percentage growth. We find a point estimate of `r round(irf_result$irf$dif_log_expenditure[1, 3],2)`, suggesting that a standard deviation shock to the percentage growth of expenditure is associated with a reduction in GDP growth of `r cat(round(irf_result$irf$dif_log_expenditure[1, 3],2) * 100, "%")` **(-22% of a standard deviation?)**. The confidence interval for this IRF is `r  cat("(",round(irf_result$Lower$dif_log_expenditure[1, 3],2), ",", round(irf_result$Upper$dif_log_expenditure[1, 3],2), ")")` - ( -0.29 , -0.08 ). As 0 is not contained in this interval, it suggests that there is a statistically significant crowding out effect of government expenditure. Nevertheless, tracing the IRF out over time, by the next quarter the cumulative response is positive,  stabilising at a value of `r round(irf_result$irf$dif_log_expenditure[11, 3],2)` by the 10th quarter. However, the percentile intervals for these later IRFs contain 0 and thus these results are not statistically significant. 


Considering now the response of GDP to a shock to government revenues, we find no effect in the quarter of the shock. However, the following 4 quarters see positive responses, with the cumulative IRF peaking a year after the initial shock and remaining at this level. Based on the bootstrap confidence interval, this effect is statistically significant. Additionally, the response of a positive shock to government revenue is itself positive, conflicting with the results from the literature. We interpret this in our discussion. 


However, the IRFs report the response of GDP growth to a shock to growth in the fiscal variables, as the scales for these variables are not equal, a conversion factor must be applied to convert the elasticities into multipliers. Various competing conversion factors have been used in the literature, Ramey (2019) provides a review, noting the limitations of approaches commonly used, instead suggesting  

There are multiple definitions for fiscal multipliers in the literature, this paper follow's Ramey (2019), defining them as "the ratio of the change in output to the change in the [fiscal variable] that caused it". This allows us to interpret our IRF results as fiscal multipliers. Clearly this definition is not complete as IRFs are dynamic functions, their value may change depending on the period considered. This paper pays special attention to 2 horizons, the instantaneous multiplier, and the multiplier after 10 quarters. While the dynamics of the fiscal multipliers could be expected to last beyond 10 quarters, we expect the majority of the response of output to a transitory shock to the fiscal variables to be captured within this horizon. Furthermore, extrapolating to further horizons would reduce the confidence with which we could report our results. Therefore
...
We now consider the results for the FEVD. Unlike the IRFs, which traces out the dynamic responses of variables to a transitory shock to one of the model variables, the FEVD measures the proportion of forecast error variance in a variable that is attributable to each shock. For GDP, we find that `r paste0(round(FEVD_result$dif_log_GDP[1,3] * 100, 0), "%")` of variability is captured by itself, a figure that drops to `r paste0(round(FEVD_result$dif_log_GDP[10,3] * 100, 0), "%")` by the 10th quarter. This is a relatively large share, suggesting that GDP growth is largely driven by its own dynamics. Considering now the share of the FEVD for GDP attributed to the fiscal variables at this horizon, we find they both contribute similarly to the variability in GDP growth, with FEVD shares of  
`r paste0(round(FEVD_result$dif_log_GDP[10,1] * 100, 1), "%")` and `r paste0(round(FEVD_result$dif_log_GDP[10,5] * 100, 1), "%")` for government expenditure and revenue respectively.

Recalling the VAR in differences specification with logarithms,
...
The IRFs for GDP to a shock to government revenue however are a much greater magnitude, still positive, the IRF peaks at 1.5, again a year after the initial shock. 
