...
# Convert variables to per capita, note revenue, expenditure, and GDP are in £ million so need to multiply. Doing this
  ...
  # Seasonal Adjustment of data using X-13ARIMA-SEATS
   mutate(Revenue_SA = final(seas(ts(Revenue_real, start = min(Year), frequency = 4))),
         ...
         mean_SR_Rate = final(seas(ts(mean_SR_Rate, start = min(Year), frequency = 4))),
         ERI = final(seas(ts(ERI, start = min(Year), frequency = 4)))
         )  %>%
 # convert back to numeric
    ...
    mean_SR_Rate = as.numeric(mean_SR_Rate),
    ERI = as.numeric(ERI)
...
Here the vector \(X_t \) defines the endogenous variables, as previously mentioned, we have assumed a lag order  ($p$) of 4. The term \( \mu \) in equation (1) captures the deterministic component of the model. We observe trending behavior between the fiscal variables, GDP, and the GDP deflator. However,  given that this same trend is not observed for the short term interest rate and the ERI, incorporating such a trend could lead to misspecification in the model. Instead, this pattern may reflect unit roots in the data, which many macroeconomic series are argued to possess. Given these potential unit roots, it may be appropriate to consider the VECM representation for the reduced form model. While unit root tests are typically performed to motivate cointegration testing, Maddala and Kim (1998) note the limitations with this approach, namely that these battery of tests are not independent, and thus their size are not as desired.  Instead, Kilian and Lutkepohl (2016) propose that researchers are explicit when imposing when imposing unit roots; for this analysis, we consider all variables to be integrated of order 1. We revisit the matter of the deterministic component once we have defined the specification for the reduced form.

Kilian and Lutkepohl (2016) show that the VAR model can be reparameterised as a VECM by subtracting the lagged variables and rearranging terms:  
...
chow_stat <- function(Y, T_break, p) {
  tryCatch({
    ...
    model_restricted <- VAR(Y, p = p, type = "none")
    ...
    model_pre <- VAR(Y_pre, p = p, type = "none")
    model_post <- VAR(Y_post, p = p, type = "none")
...
Through this process we can derive the empirical distribution of the bootstrap time series. We then compute the pseudo p-value as the percentage of times the bootstrap statistic exceeds the test statistic. Using `r Rep` replications, we obtain a p-value of `r round(p_val, 3)`. This can be seen in figure 3, where only `r round(p_val, 3) * Rep` bootstrap observations exceed our test statistic from the original data. This supports the previous asymptotic test of parameter instability. We therefore estimate the SVAR on the post-GFC sample to determine whether there are differences in the estimated fiscal multipliers in the most recent period. 
...
```{r IRF-comp-Expenditure, fig.cap="Marginal Response of GDP to Government Spending Compared \n (Restricted vs Unrestricted Model)", fig.align='center'}
...
```{r IRF-comp-Revenue, fig.cap="Marginal Response of GDP to Government Revenue Compared \n (Restricted vs Unrestricted Model)", fig.align='center'}
# Plot Revenue IRFs
...
```{r IRF-comp-Expenditure_all, fig.cap="Marginal Response of GDP to Government Expenditure Compared  \n (Baseline vs Reduced Lags)", fig.align='center'}
# Plot Expenditure IRFs
...
```{r IRF-comp-Revenue_all, fig.cap="Marginal Response of GDP to Government Revenue Compared \n (Baseline vs Reduced Lags)", fig.align='center'}
# Plot Revenue IRFs
...
For the government expenditure shock, we find similar trends under both specifications, with both models reporting an instantaneous drop in GDP growth, followed by an increase the next quarter. There are some discrepancies with the point estimates from the 2nd quarter onward, with the reduced lag specification reporting lower marginal responses. Given the wide confidence intervals reported in the baseline specification, this does not challenge our results.  

Regarding the IRF of GDP following a shock to government revenue, we observe a similar peak the quarter after the shock.  Following this, the marginal response decreases over time, however this is more gradual, and does not show a reversal as with the baseline model. This suggests that the reduced lag has not been able to capture the dynamics of the underlying response; the motivation for our use of a lag order chosen a priori.
...
Table 1 summarises the multipliers reported by this paper. For government spending we find little evidence of a crowding in effect, with weak a positive multiplier in our baseline specification that is robust over the period following the GFC. Using a reduced lag specification, we find a negative multiplier. Although we highlight concerns that too few lags would not allow us to adequately recover the IRFs, the point estimates under this specification are within the confidence interval for our baseline model. Thus this multiplier is compatible with our baseline results. In contrast, we found a statistically significant and positive multiplier for government revenue. These results were determined to be robust to our checks for potential misspecification, despite some evidence of the the government revenue multiplier being larger following the GFC. Furthermore, positive government revenue multipliers are not consistent with the findings of the literature. We consider potential explanations for this.

`r kable(long_table, caption = "Present Value Multipliers by Specification and Fiscal Instrument")`

De Castro (2006) finds a positive response of GDP to a shock to net taxes, which they attribute to a mediating effect via increased government expenditure in response to the tax shock. Given that we have reported either weak or negative government expenditure multipliers, this transmission mechanism would not apply to our analysis. Instead we discuss alternative hypotheses.


Recalling that the government revenue variable is defined following the ESA (2010), this can be interpreted as net taxes, as has also been outlined by the ONS. Thus the positive multiplier for government revenue is counter intuitive as we would expect GDP to decline in response to greater taxation. However, it is worth highlighting that tax revenues, as captured by this analysis are not the same as the fiscal instrument available to the government: tax legislation encompassing tax rates, allowances and exemptions. Thus one explanation for the findings of positive tax multipliers is that this is reflecting the UK's position on the Laffer curve, the relationship between tax rates and GDP may be positive as expected, however lower tax rates may lead to greater overall revenue by providing a greater incentive for labour and the use of capital. Simultaneously, lower tax rates would be expected to increase GDP, the combination of these two effects could be responsible for the positive multiplier. Nevertheless, Nguyen, Onnis, and Rossi (2020) analysed the effects of unexpected changes to income and consumption taxes in the UK. Their results suggested that the UK is still on the increasing portion of the Laffer curve for these taxes. As we find that these taxes account for 60% of government revenue in our sample, this does not reject the potential for the UK to be on the decreasing portion of the Laffer curve when taxes are considered as an aggregate, although it does suggest that this reason is less likely. 

Before considering the second hypothesis for our result of a positive government revenue multiplier, we highlight again the twofold objectives of this paper: to estimate fiscal multipliers for the UK and to address the concerns raised by methodological differences affecting estimates of fiscal multipliers (Ramey, 2016; Gechert, 2017). While it is not clear whether the UK has reached the decreasing portion of the Laffer curve, we still emphasise the limitations of the government revenue variable used in this analysis as well as elsewhere in the literature. This variable does not reflect a feasible policy option for the government, instead more direct measures of these options should be used. Nevertheless, this does not invalidate the identifying assumptions for government expenditure shocks. Thus, as intended, this paper has estimated fiscal multipliers, while noting concerns with an approach commonly seen in the literature. We consider the scope for future research later in this section.
...
percentiles <- quantile(exp_change_df$expenditure_change, probs = c(0.9 ,0.95), na.rm = TRUE)
percentile_df <- data.frame(
  percentile = c("90th percentile", "95th percentile"),
  value = c(percentiles[1], percentiles[2])
)
...
ggplot(exp_change_df, aes(x = expenditure_change)) +
  ...
  scale_color_manual(values = c("90th percentile" = "#E74C3C", "95th percentile" = "#27AE60")) +
  labs(
    title = "Density Plot of Annual Expenditure Change",
...
This paper has employed a SVAR analysis of fiscal multipliers using a 6-dimensional VAR with quarterly data. We use recursive short run restrictions, leveraging economic theory to determine the transmission of shocks across the VAR model thus identifying the structural parameters. We find a weak positive expenditure multiplier, whereas for government revenue we report a positive multiplier with a 68% bootstrap confidence interval excluding 0. We perform tests for misspecification to assess the robustness of these results. Across these specifications we find similar, positive results for the multiplier of government revenue. For our government expenditure multiplier, this is robust to our assessment of stability following the GFC, although we observe a modest crowding out effect when considering the reduced lag representation. Given that the IRFs from this specification are contained within the confidence interval of our benchmark model, we note that this multiplier is feasible under our main results. We discuss the policy implications of these results. The low reported multipliers for government expenditure suggest scope for the UK government to reduce its spending to improve its fiscal position, without facing counterproductive effects through a disproportionate decrease in GDP. We also consider the magnitude of interventions proposed in the literature; noting that, based on the variability of the annual change in expenditure, we would expect these to be "modest policy interventions". Thus we expect that these results would not be made redundant by agents updating their beliefs. This paper also highlights limitations of the net tax variable commonly used in the literature, and suggests scope for future research. 
...
Cheung, Y.W. and Lai, K.S. (1993). Finite-Sample Sizes of Johansen’s Likelihood Ratio Tests for Cointegration. *Oxford Bulletin of Economics and statistics*.** 55**(3), p.1025.


Cloyne, J. (2013). Discretionary Tax Changes and the Macroeconomy: A New Narrative Evidence from the United Kingdom. *American Economic Review* .**103** (4), pp1507-1528
...
International Monetary Fund. (2023). Fiscal Monitor: On the Path to Policy Normalization. Washington, DC: IMF, April.


International Monetary Fund. (2024). Fiscal Monitor: Putting a Lid on Public Debt. Washington, DC: IMF, October.
